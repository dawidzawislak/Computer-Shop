...
else
{
	if(isset($_POST['id']))
	{
		$_SESSION['id_dysk'.$_SESSION['disk_num']] = $_POST['product_id'];
		if($_SESSION['disk_num'] == 1) 
		{
			$_SESSION['disk_num'] = 2;
			header('location:disk.php');
		}
...
<head>
	...
	<title>Sklep Komputerowy - <?php echo (isset($_SESSION['config']) ? "Konfigurator PC" : "Dyski")?></title>
...
<body>
	...
	<?php 
			if(!isset($_SESSION['config']))
			{
				...
				DrawProgressBar(isset($_SESSION['buy_now']), 5+$_SESSION['disk_num']);
			}
		?>
				...
				<?php
					...
					if($result->num_rows > 0)
					{
						while($row = $result->fetch_assoc())
						{
							...
							CreateProductDiv($row['id_dysk'], $row['firma'].' '.$row['model'], $row['img_path'], $row['cena'], $row['ilosc_sztuk'], $props, isset($_SESSION['config']));
						}
					}
					else
