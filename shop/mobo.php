...
<head>
	...
	<title>Sklep Komputerowy - <?php echo (isset($_SESSION['config']) ? "Konfigurator PC" : "Płyty główne")?></title>
	...
	if(!isset($_SESSION['config']))
	{
		echo '<div id="nav">';
			echo '<ul>';
				...
				echo '<li>	<a href="#">Płyty główne				</a></li>';
				echo '<li>	<a href="cpu.php">Procesory    			</a></li>';
		...
		DrawProgressBar(isset($_SESSION['buy_now']), 1);
	}
	?>
	<div id="content">
		<?php
		if(isset($_SESSION['config']))
		{
			echo '<div id="config_h">';
				echo '<form method="POST" action="">';
					...
					echo '<input style="width: 80px; margin: 0;  margin-left: 85px;" type="submit" name="skip" value="Pomiń">';
				echo '</form>';
			echo '</div>';
		}
		?>
			...
			<?php
				...
				if($result->num_rows > 0)
				{
					while($row = $result->fetch_assoc())
					{
						...
						CreateProductDiv($row['id_MOBO'], $row['firma'].' '.$row['model'], $row['img_path'], $row['cena'], $row['ilosc_sztuk'], $props, isset($_SESSION['config']));
					}
				}
				else
