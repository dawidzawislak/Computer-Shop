...
$_SESSION['table_name'] = "procesory";
 ...
 <body>
		...
		<?php DrawProgressBar(isset($_SESSION['buy_now']), 2); ?>
		<div id="content">
			<div id="config_h">
			...
			<form method="POST" action="">
				...
				<?php
					...
					CreateCheckboxesForProperty("Taktowanie [GHz]", "taktowanie", $_SESSION['table_name'], $conn, "socket LIKE \"".$_SESSION['socket']."\"");
					CreateCheckboxesForProperty("Ilość rdzeni", "ilosc_rdzeni", $_SESSION['table_name'], $conn, "socket LIKE \"".$_SESSION['socket']."\"");
					CreateCheckboxesForProperty("TDP [W]", "TDP", $_SESSION['table_name'], $conn, "socket LIKE \"".$_SESSION['socket']."\"");
		...
		<div id="products">
			<?php
				...
				$query = "SELECT * FROM ".$_SESSION['table_name']." WHERE socket LIKE \"".$_SESSION['socket']."\" AND";
				...
				if($result->num_rows > 0)
				{
					while($row = $result->fetch_assoc())
					{
						...
						CreateProductDiv($row['id_CPU'], $row['firma'].' '.$row['model'], $row['img_path'], $row['cena'], $row['ilosc_sztuk'], $props);
					}
				}
				else
