...
<body>
	...
	<?php DrawProgressBar(isset($_SESSION['buy_now']), 4); ?>
	<div id="content">
		<div id="config_h">
				...
				<?php
					...
					if($conn->connect_error)
						die("Connection to \"sklep\" failed: ".$conn->connect_error);
													...
													CreateCheckboxesForProperty("Moc [W]", "moc",  $_SESSION['table_name'], $conn, "moc > ".(($_SESSION['power_consump']+$_SESSION['power_consump2']) / 0.80));
							CreateCheckboxesForProperty("Certyfikat", "certyfikat",  $_SESSION['table_name'], $conn, "moc > ".(($_SESSION['power_consump']+$_SESSION['power_consump2']) / 0.80));
		...
		<div id="products">
			<?php
				...
				$query = "SELECT * FROM ".$_SESSION['table_name']." WHERE moc > ".(($_SESSION['power_consump']+$_SESSION['power_consump2']) / 0.75)." AND";
				...
				if($result->num_rows > 0)
				{
					while($row = $result->fetch_assoc())
					{
						...
						CreateProductDiv($row['id_PSU'], $row['firma'].' '.$row['model'], $row['img_path'], $row['cena'], $row['ilosc_sztuk'], $props);
					}
				}
				else
