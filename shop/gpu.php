...
<head>
	...
	<title>Sklep Komputerowy - <?php echo (isset($_SESSION['config']) ? "Konfigurator PC" : "Karty graficzne")?></title>
		...
		if(!isset($_SESSION['config']))
		{
			echo '<div id="nav">';
				echo '<ul>';
					...
					echo '<li>	<a href="#">Karty Graficzne   			</a></li>';
			...
			DrawProgressBar(isset($_SESSION['buy_now']), 3);
		}
		?>
				...
				<?php
					...
					if($result->num_rows > 0)
					{
						while($row = $result->fetch_assoc())
						{
							...
							CreateProductDiv($row['id_GPU'], $row['firma'].' '.$row['model'], $row['img_path'], $row['cena'], $row['ilosc_sztuk'], $props, isset($_SESSION['config']));
						}
					}
					else
