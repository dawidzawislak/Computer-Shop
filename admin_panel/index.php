...
<head>
	...
	<title>Panel Administracyjny - Nowe zamówienia</title>
        ...
        $conn = new mysqli($host, $dbuser, $dbpass, $dbname);
          ...
          while($row = $result->fetch_assoc())
          {
            if($row["wyslano"] == 0)
            {
              if(!$print_tab)
              {
                echo "<table><thead><tr><th>Id</th><th>Klient</th><th>Data zamówienia</th><th>Cena zamówienia[zł]</th><th>Data realizacji</th><th>Uwagi</th><th>Produkty</th></tr></thead><tbody>";
                $print_tab = TRUE;
              }
              echo "<tr>";
                ...
                echo '<td><form action=realised.php method="POST">';
                echo '<input type="date" style="margin-right: 20px;" name="date" required>';
                echo '<input type="hidden" name="id" value="'.htmlentities($row['id_zamowienia']).'">';
                echo '<input type="submit" name="Prześlij" value="Ustaw">';
                echo '</form></td>';
                echo '<td>';
                if (empty($row["uwagi"]))
                    echo "Brak";
          ...
          echo "<p>Brak nowych niezrealizowanych zamówień</p>";
